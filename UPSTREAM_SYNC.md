...
2026-08-20 对本地引用和远端 `upstream/dev` 的只读检查结果如下：

| 引用 | 提交 | 与 `beta` 的关系 |
| --- | --- | --- |
| `beta` | `e02cfb61` | 当前维护分支 |
| `origin/dev` | `fd5fba91` | 是 `beta` 的祖先；`beta` 单侧多 7 个提交 |
| 本地 `upstream/dev` | `71a2bf30` | 是 `beta` 的祖先；`beta` 单侧多 31 个提交 |
| 远端 `upstream/dev` | `758f07d5` | 与 `beta` 已分叉：`beta` 侧 31、上游侧 4 个提交 |
| `upstream-v0.70.1^{}` | `fa3bcca2` | 与 `beta` 已分叉：`beta` 侧 52、tag 侧 12 个提交 |

最新上游候选以临时引用完成了合成树测试：当前 XUDP 修改叠加远端
`758f07d5` 后，Docker 全量相关包测试、目标包 Race Detector 和 frps/frpc
构建均通过，且没有直接路径冲突。该候选尚未正式合入 `beta`；完成当前改动并使
工作树干净后，仍须按第 4 节流程执行真实同步。不需要合并
`upstream-v0.70.1`。
